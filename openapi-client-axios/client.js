...
var OpenAPIClientAxios = /** @class */ (function () {
    ...
    function OpenAPIClientAxios(opts) {
            ...
            return __generator(this, function (_c) {
                switch (_c.label) {
                    ...
                    case 3:
                        ...
                        _b = this;
                        return [4 /*yield*/, swagger_parser_1.default.dereference(lodash_1.default.cloneDeep(this.document), this.swaggerParserOpts)];
                    case 4:
                        // dereference the document into definition
            ...
            for (var path in _this.definition.paths) {
                if (_this.definition.paths[path]) {
                    ...
                    for (var m in methods) {
                        if (methods[m] && lodash_1.default.includes(Object.values(client_1.HttpMethod), m)) {
                            var method = m;
                            var operation = lodash_1.default.find(_this.getOperations(), { path: path, method: method });
        ...
        this.validateDefinition = function () {
            var _a = openapi_schema_validation_1.validate(_this.document, 3), valid = _a.valid, errors = _a.errors;
            if (!valid) {
        ...
        this.getBaseURL = function (operation) {
            ...
            for (var _b = 0, _c = baseURLBuilder.names; _b < _c.length; _b++) {
                var name = _c[_b];
                var varValue = _this.baseURLVariables[name];
                if (varValue !== undefined) {
                    // if varValue exists assign to baseURLVariablesResolved object
                    if (typeof varValue === 'number') {
                        // if number, get value from enum array
                        var enumVal = baseURLVariableSet[name].enum[varValue];
                        if (enumVal) {
                            baseURLVariablesResolved[name] = enumVal;
                        }
                        else {
                            // if supplied value out of range: throw error
                            throw new Error("index " + varValue + " out of range for enum of baseURL variable: " + name + ";               enum max index is " + (baseURLVariableSet[name].enum.length - 1));
                        }
                    }
                        ...
                        if (baseURLVariableSet[name].enum.includes(varValue)) {
                            ...
                            throw new Error(varValue + " is not a valid entry for baseURL variable " + name + ";                 variable must be of the following: " + baseURLVariableSet[name].enum.join(', '));
                        }
                    }
                }
                else {
        ...
        this.getOperations = function () {
            ...
            return lodash_1.default.chain(paths)
                ...
                var methods = lodash_1.default.pick(pathObject, lodash_1.default.values(client_1.HttpMethod));
        ...
        var optsWithDefaults = __assign(__assign({ validate: true, strict: false, quick: false, withServer: 0, baseURLVariables: {}, swaggerParserOpts: {} }, opts), { axiosConfigDefaults: __assign({ paramsSerializer: function (params) { return query_string_1.default.stringify(params, { arrayFormat: 'none' }); } }, (opts.axiosConfigDefaults || {})) });
